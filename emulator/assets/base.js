...
/* --- Videopac Vault: O2EM keybind fixes ---------------------------------
 * webretro's stock binds put RetroPad buttons on letter keys. The o2em core
 * maps those buttons to the console's NUMBER keys (see retro_load_game's
 * input descriptors in libretro.c):
 *     X -> Numeric 0    L -> Numeric 1    R -> Numeric 2
 *     L2 -> Numeric 3   R2 -> Numeric 4   L3 -> 5   R3 -> 6
 * With the stock binds ("y","e","p","r","o") every one of those letters typed
 * a stray digit into the console at the same time. That silently broke every
 * game driven from the alphanumeric keyboard - e.g. K.C. Munchkin's
 * create-a-maze mode (RESET then P): "p" was bound to R, so pressing P sent
 * P *and* 2, and the 2 started a normal game instead of the maze editor.
 * Verified against the core by running the ROM headlessly: clean P opens the
 * editor, P+2 does not.
 * These buttons are redundant anyway - the real number keys 0-9 already reach
 * the console directly as RETRO_DEVICE_KEYBOARD - so they're unbound here.
 *
 * input_player1_select (Show/Hide the on-screen console keyboard) was on
 * "space", but SPACE is a real key on the Odyssey2 keyboard, so it could
 * never be typed. It's on "rshift" now - right shift exists on every Mac
 * keyboard (unlike Insert) and isn't a key on the Odyssey2 keyboard, so it
 * can't collide with typing. There's also a "Console keyboard" item in the
 * More menu for anyone who'd rather click.
 * ---------------------------------------------------------------------- */
var defaultKeybinds = 'input_player1_start = "enter"\ninput_player1_select = "rshift"\ninput_player1_l = "nul"\ninput_player1_l2 = "nul"\ninput_player1_r = "nul"\ninput_player1_r2 = "nul"\ninput_player1_a = "nul"\ninput_player1_b = "g"\ninput_player1_x = "nul"\ninput_player1_y = "t"\ninput_player1_up = "up"\ninput_player1_left = "left"\ninput_player1_down = "down"\ninput_player1_right = "right"\ninput_player1_l_x_minus = "nul"\ninput_player1_l_x_plus = "nul"\ninput_player1_l_y_minus = "nul"\ninput_player1_l_y_plus = "nul"\ninput_player1_l3_btn = "x"\ninput_player2_up = "w"\ninput_player2_left = "a"\ninput_player2_down = "s"\ninput_player2_right = "d"\ninput_player2_b = "q"\ninput_player1_r_x_minus = "nul"\ninput_player1_r_x_plus = "nul"\ninput_player1_r_y_minus = "nul"\ninput_player1_r_y_plus = "nul"\ninput_player1_r3_btn = "comma"\ninput_menu_toggle = "f1"\ninput_save_state = "f2"\ninput_load_state = "f3"\ninput_screenshot = "f4"\ninput_hold_fast_forward = "nul"\ninput_toggle_fast_forward = "nul"\ninput_hold_slowmotion = "nul"\ninput_toggle_slowmotion = "nul"\ninput_grab_mouse_toggle = "backslash"\ninput_game_focus_toggle = "tab"\n';
...
// C64 demoscene productions all use custom fastloaders, so every one of them
// needs true drive emulation - see the Demoscene category in the catalogue.
var viceTrueDriveEmulationRoms = ["wasteland", "c64_demo_dutchbreeze", "c64_demo_deusexmachina", "c64_demo_royalarte", "c64_demo_wonderland12"];
...
var installedCores = ["a5200", "freechaf", "freeintv", "gearcoleco", "genesis_plus_gx", "handy", "mednafen_ngp", "mednafen_psx_hw", "mednafen_vb", "mednafen_wswan", "melonds", "mgba", "mupen64plus_next", "neocd", "nestopia", "o2em", "opera", "parallel_n64", "prosystem", "snes9x", "stella2014", "vecx", "vice_x64", "virtualjaguar", "yabause"];
...
// key press stuff
/* Videopac Vault: synthesized events also carry keyCode/which. RetroArch's
 * emscripten input handler reads the legacy keyCode field, so events built
 * from `code` alone were being ignored - which is why the menu's synthesized
 * key presses (and webretro's own ctrl+V paste) did nothing. keyCode isn't
 * settable through the constructor, hence defineProperty. */
function fakeKey(type, info) {
	var e = new KeyboardEvent(type, {code: info.code || undefined, key: info.key || undefined, shiftKey: info.shiftKey || undefined, bubbles: true});
	var legacy = info.keyCode;
	if (legacy === undefined && info.code && typeof charToCodeMap === "object") {
		for (var ch in charToCodeMap) {
			if (charToCodeMap[ch] && charToCodeMap[ch].code === info.code && charToCodeMap[ch].keyCode !== undefined) {
				legacy = charToCodeMap[ch].keyCode;
				break;
			}
		}
	}
	if (legacy !== undefined) {
		try {
			Object.defineProperty(e, "keyCode", {get: function () { return legacy; }});
			Object.defineProperty(e, "which",   {get: function () { return legacy; }});
		} catch (err) {}
	}
...
function grab(url, type, success, fail) {
	...
	// Local-server cache-buster: python http.server sends no cache headers,
	// so Chrome heuristically caches ROM/BIOS files and serves stale copies
	// after they change on disk. Force a fresh fetch every time.
	url += (url.indexOf("?") === -1 ? "?" : "&") + "cb=" + Date.now();
...
function readyRomFetch() {
	/* Videopac Vault: queries.rom has already been through decodeURIComponent
	 * (see `queries` near the top), so it's the raw filename. Dropping that
	 * straight into a URL breaks on any name containing "#" - the browser
	 * treats it as the start of a fragment, so
	 *     roms/VP05 - Mod of VP#05 Blackjack (Mod).bin
	 * is requested as
	 *     roms/VP05 - Mod of VP
	 * and 404s with "Could not get ROM". 17 of the "Mod of VP#nn" carts were
	 * unplayable because of this. Re-encode per path segment so "/" still
...
/* Videopac Vault: saved keybinds in localStorage are merged OVER the defaults,
 * so a set saved before the O2EM keybind fixes above would silently reinstate
 * the broken mapping (letters doubling as console number keys) and there'd be
 * no way to tell from the UI. Stamp the default set with a version and drop any
 * saved binds from an older stamp. Bump this when defaultKeybinds changes. */
var KEYBIND_SET_VERSION = "vault-2";
if (localStorage.getItem("VideopacVault_keybind_version") !== KEYBIND_SET_VERSION) {
	localStorage.removeItem("RetroArch_settings_keybinds");
	localStorage.setItem("VideopacVault_keybind_version", KEYBIND_SET_VERSION);
}
...
function afterStart() {
	...
	/* Videopac Vault: clickable stand-ins for the two console keys that sit on
	 * awkward physical keys. RESET is hardwired to F5 inside the o2em core
	 * (libretro.c checks RETROK_F5), and Mac function keys are system keys, so
	 * synthesize the keypress instead of asking the user to press it. The
	 * on-screen keyboard toggle follows whatever input_player1_select is bound
	 * to, so a rebind in Controls keeps working. */
	/* RESET: don't synthesize F5 - all the core does with F5 is call
	 * retro_reset(), which is exactly what _cmd_reset() triggers. Calling it
	 * directly can't be defeated by key handling or OS function-key mapping. */
	var o2ResetKey = document.getElementById("o2resetkey");
	if (o2ResetKey) o2ResetKey.onclick = function() {
		Module._cmd_reset();
	};
	/* The on-screen keyboard has no such command - it toggles on a RetroPad
	 * SELECT press - so this one does have to go through a key event. */
	var o2KbdToggle = document.getElementById("o2kbdtoggle");
	if (o2KbdToggle) o2KbdToggle.onclick = function() {
		var code = configIDToCode(savedKeybindsObj["input_player1_select"]);
		if (code) fakeKeyPress({code: code});
	};
		...
		if (wasmReady && bundleReady && biosReady) {
			...
			switch (core) {
				...
				case "vice_x64":
					...
					// disk speed - Bruce Lee sat on SEARCHING/LOADING for a minute.
