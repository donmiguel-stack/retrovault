...
window.GAMEPAGES_DATA = {
 ...
 "new_bird-hunt": {
  ...
  "keys": "j2arrows",
  "manual": {
   "text": "<h3>Playing</h3><p>Move the crosshair with the joystick and press fire to shoot, letting the fire button go between shots. Three birds appear in each round, one at a time, each on a different flight path, and you have five shots to spend on them. A bird you hit drops out of the sky; a miss shows a brief white flash. The five yellow cartridges in the top right are your shots — each one you fire takes a cartridge away, and all five come back at the start of the next round.</p><p>Every round, the first one included, opens with a two-second wait, then a buzz as the start signal, then one more second. From the second round on, that wait begins only once the round tune has finished. While it runs the birds are hidden and you cannot shoot, but the crosshair still moves. Fire pressed on the title screen or during that wait costs you nothing — let it go once the round starts and press again to take your first shot.</p><h3>Scoring</h3><p>The total is brought up to date at the end of each round:</p><table><tr><th>Birds hit</th><th>Result</th></tr><tr><td>0</td><td>Total halved, rounded down</td></tr><tr><td>1</td><td>+1 point</td></tr><tr><td>2</td><td>+5 points</td></tr><tr><td>3</td><td>+10 points, plus 5 points for every cartridge still unused</td></tr></table><p>The score stops climbing at 65535. Three hits play a happy tune, and with two cartridges left over it plays three times. If halving takes the total to zero — which it does from 0 or 1 point as well — that is game over: an alarm sounds, two seconds pass, and the title screen comes back, sitting lower on the screen than before.</p><h3>Pausing</h3><p>A key on the console keyboard pauses the game. Round timers and bird movement stop while it is paused. Fire starts it again without spending a shot; let fire go before you shoot. A key held down does not pause the game a second time after it resumes — you have to lift it and press it again.</p><h3>The playfield</h3><p>Blue sky over green grass. On a G7400 the cartridge draws its own scenery over that — a tree, a hedge line and tufts of grass — which is what you see here, since the Vault boots Videopac titles with the G7400 BIOS.</p>",
   "source": "the game’s own README",
   "url": "https://github.com/Ahnl66/BIRD-HUNT#spelen"
  },
  ...
  "history": {
   "text": "A 2026 homebrew written from scratch in Intel 8048 assembly and published on GitHub by Ahnl66 — source, build file and the author's own emulator test harness included, under the working title ArtGame that the test names still carry. It is built to run on the real console from a PicoPac cartridge, and it knows which machine it is on: a plain G7000 gets blue sky over green grass, while a G7400 is recognised by two opcode signatures in its Plus BIOS and gets a landscape laid over that playfield — a tree, a hedge line and tufts of grass. The cartridge is four standard 2 KB banks, the game itself in the last one and a slice of the background art plus a copy of the loader in each of the other three. The author reports both G7000 modes and G7400 PAL tested in emulation, with a test on real G7400 hardware still to come.",
