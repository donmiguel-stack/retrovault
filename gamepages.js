...
window.GAMEPAGES_DATA = {
 ...
 "new_bird-hunt": {
  "input": "joystick",
  "keys": "j2arrows",
  "note": "This cartridge reads <strong>joystick&nbsp;2</strong>, and it also watches the console keyboard — which is why the Vault puts joystick 2 on the <span class='kbd'>arrow keys</span> with <span class='kbd'>Left Ctrl</span> to fire on this page only. (The usual joystick-2 keys, <span class='kbd'>W</span> <span class='kbd'>A</span> <span class='kbd'>S</span> <span class='kbd'>D</span> and <span class='kbd'>Q</span>, are all real keys on the console's own keyboard, and this game pauses on a console key — so they would freeze the crosshair instead of moving it.) Let fire go between shots: only a fresh press counts, which is why pressing it on the title screen or during the count-in before a round costs you nothing. Every round sends three birds past, one at a time, on paths that change each time, and gives you five shots — the five yellow cartridges in the top right. Hitting all three scores 10 points plus 5 for every cartridge still unused, two hits score 5, one scores 1, and missing all three halves your total; if that halving takes you to zero the game is over. Any key on the console keyboard pauses the round, and fire resumes it without spending a shot. If you are on a copy of the Vault downloaded before 19 September 2026 and the arrow keys do nothing here, download the Vault again — this control fix lives in the emulator, which the in-app Update deliberately never replaces.",
  "history": {
   "text": "A 2026 homebrew written from scratch in Intel 8048 assembly and published on GitHub by Ahnl66, source, build file and emulator test harness included. It runs on a plain G7000 as blue sky over green grass, but on a G7400 it recognises the Plus BIOS and lays its own landscape over that playfield — a tree, a hedge line and tufts of grass. The cartridge is four standard 2 KB banks: the game itself sits in the last one, the other three each carry a slice of the background art and a copy of the loader that puts it on screen.",
