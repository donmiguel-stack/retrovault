...
window.GAMEPAGES_DATA = {
 ...
 "new_bird-hunt": {
  "input": "joystick",
  "keys": "j2arrows",
  "note": "Aim with the <span class='kbd'>arrow keys</span>, fire with the backtick key <span class='kbd'>&#96;</span> (left hand, top-left of the keyboard). This cartridge reads <strong>joystick&nbsp;2</strong> and also watches the console keyboard, so the Vault remaps joystick 2 onto those keys on this page only. That is also why fire is not on the usual <span class='kbd'>G</span>: the game scans all six rows of the console keyboard and pauses on any key it finds down, without checking which one — and <span class='kbd'>G</span>, every other letter, <span class='kbd'>Space</span>, <span class='kbd'>.</span>, <span class='kbd'>/</span>, <span class='kbd'>-</span> and <span class='kbd'>=</span> are all real keys on that keyboard, so they freeze the crosshair instead of moving it. Let fire go between shots: only a fresh press counts, which is why pressing it on the title screen or during the count-in before a round costs you nothing. Every round sends three birds past, one at a time, on paths that change each time, and gives you five shots — the five yellow cartridges in the top right. Hitting all three scores 10 points plus 5 for every cartridge still unused, two hits score 5, one scores 1, and missing all three halves your total; if that halving takes you to zero the game is over. A console key also pauses a round in progress, and fire resumes it without spending a shot. If you are on a copy of the Vault downloaded before 19 September 2026 and the arrow keys do nothing here, download the Vault again — this control fix lives in the emulator, which the in-app Update deliberately never replaces.",
  "history": {
