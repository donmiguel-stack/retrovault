...
FILES = [
    ...
    # news.js feeds the ticker tape on top of the homebrew panel; index.html
    # references it, so the two have to travel together through an update.
    "news.js",
    ...
    # so they MUST ship together - an install that took the new game.html
    # without these two would render a blank page. catalogue.html is the flat
    # index of every game that the same generator writes.
    "game.css", "game-page.js", "catalogue.html",
    "favicon.ico", "icon-16.png", "icon-32.png", "icon-48.png", "apple-touch-icon.png",
    "README.md", "LICENSE.md",
    # Search-engine files. sitemap.xml is generated by tools/make_sitemap.py -
    # rerun that whenever games are added, or the new pages stay undiscoverable
    # (the shelf grid is built in JS, so crawlers can't find them by following
    # links).
    "robots.txt", "sitemap.xml",
]
# Deliberately NOT here: game/, the 396 pre-rendered pages from
# tools/make_gamepages.py. They exist so that search engines see each game's
# own title, description and canonical in the served HTML; an offline copy
# reaches the same pages through game.html?id= and gains nothing from 4 MB of
# duplicates. serve.py's ALLOWED_DIRS would refuse them anyway - a new folder
# can never reach an install that already exists.
